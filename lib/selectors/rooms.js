...
var usersSelector = exports.usersSelector = function usersSelector(state) {
  return state.users.users;
};
var filtersSelector = exports.filtersSelector = function filtersSelector(state) {
...
var getIndexUsers = exports.getIndexUsers = function getIndexUsers(hotelUsers) {
  return (0, _collection.keyBy)(hotelUsers || [], '_id');
};

var getUserTasks = exports.getUserTasks = function getUserTasks(hotelTasks, userId) {
...
var getComputedRooms = exports.getComputedRooms = function getComputedRooms(hotelRooms, indexFloors, indexRoomStatuses, indexRoomHousekeepings, groupedCalendar, indexPlanning, groupedNotes, groupedTasks, isPlanned, roomsUpdates, userId) {
  if (!hotelRooms || !hotelRooms.length) {
    return [];
  }

  var mappedRooms = (0, _collection.map)(hotelRooms, function (room) {
    var roomId = (0, _object.get)(room, '_id');
    var roomPlanning = (0, _object.get)(indexPlanning, roomId, {});
  ...
  return (0, _collection.sortBy)(mappedRooms, 'name');
};

var getComputedRoomsIndex = exports.getComputedRoomsIndex = function getComputedRoomsIndex(hotelRooms) {
  return (0, _collection.keyBy)(hotelRooms, '_id');
};

var getAvailableFloors = exports.getAvailableFloors = function getAvailableFloors(hotelRooms, hotelFloors) {
  if (!hotelRooms || !hotelRooms.length) {
    return [];
  }
...
var getActiveRoom = exports.getActiveRoom = function getActiveRoom(hotelRoomsIndex, activeRoom) {
  return (0, _object.get)(hotelRoomsIndex, activeRoom, null);
};

var getCatalogByActiveRoom = exports.getCatalogByActiveRoom = function getCatalogByActiveRoom(hotelCatalog, activeRoom) {
...
var getPopupTasks = exports.getPopupTasks = function getPopupTasks(hotelTasks, userId) {
  if (!hotelTasks || !hotelTasks.length) {
    return [];
  }
...
var getFilteredRooms = exports.getFilteredRooms = function getFilteredRooms(hotelRooms, filters) {
  if (!hotelRooms || !hotelRooms.length) {
    return [];
  }
  ...
  if (filters.roomsSearchQuery) {
    (function () {
      var cleanQuery = filters.roomsSearchQuery.toLowerCase();
...
var computedIndexUser = exports.computedIndexUser = (0, _reselect.createSelector)([usersSelector], getIndexUsers);
